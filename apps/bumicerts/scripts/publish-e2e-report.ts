...
const PAGES_BASE_URL = "https://gainforest.github.io/bumicerts-e2e-reports";
const APP_NAME = "bumicerts";
...
function runInherit(command: string, args: string[], cwd = process.cwd()): void {
  execFileSync(command, args, { cwd, stdio: "inherit" });
}

function tryRun(command: string, args: string[], cwd = process.cwd()): string | null {
  try {
...
function getReportsRepoPath(): string {
  return join(homedir(), ".cache", "gainforest", "bumicerts-e2e-reports");
}

function ensureReportsRepo(repoPath: string): void {
...
function readIndex(indexPath: string): ReportEntry[] {
  if (!existsSync(indexPath)) return [];
  const parsed = JSON.parse(readFileSync(indexPath, "utf8")) as unknown;
  return Array.isArray(parsed) ? (parsed as ReportEntry[]) : [];
}

function writeIndexHtml(repoPath: string): void {
  writeFileSync(
  ...
  <body>
    ...
    <script>
      async function render() {
        ...
        app.className = "";
        app.innerHTML = \`<table><thead><tr><th>Created</th><th>Branch</th><th>Commit</th><th>Author</th><th>Report</th></tr></thead><tbody>\${reports.map((report) => \`<tr><td>\${new Date(report.createdAt).toLocaleString()}</td><td><code>\${report.branch}</code></td><td><code>\${report.commit}</code></td><td>\${report.author}</td><td><a href="./\${report.reportPath}/">Open report</a></td></tr>\`).join("")}</tbody></table>\`;
      }
      render();
    </script>
  </body>
...
function main(): void {
  ...
  if (!existsSync(join(reportSource, "index.html"))) {
    throw new Error(
      `No Playwright HTML report found at ${reportSource}. Run \`bun run test:e2e\` first.`,
    );
  }
